...
> 目标项目的边界检查见 `src/content/guardrails/boundary-check.md`。
...
| `src/content/` | 规范文件（.md） | TypeScript 代码 |
| `src/presets/` | preset.json + 栈特定规范 | 通用规范、代码逻辑 |
| `src/templates/` | 模板文件（.template.md） | 代码逻辑 |

### 2. src/content/ 技术栈无关性

在 `src/content/` 目录的文件中检测以下违规模式：
...
- `src/content/` 中的文件：kebab-case（`code-quality.md`）
